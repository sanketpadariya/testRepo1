...
{
    public partial class frmFinanceResponse : System.Web.UI.Page
    {
        #region Handler
        string sQS;
        string[] aQS;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (Request.QueryString.Count > 0)
                {
                    ...
                    tmpOderINS = new clsResponse().financeOrderIns(paymentID, tmpSessionUserID, trnID, createdDate, responseCode, Amount, Quentity, tax, TotalAmt);
                    if (tmpOderINS != 1)
                    {
                        convert2_xbrl.Show("Error while Inserting Finance order", this);
                    }

                    fianceBillingInformation();
                    tmpBillingINS = new clsResponse().financeBillingInfo(paymentID, bName, bAdd, bCity, bState, bZip, bEmail, bTelephone);
                    if (tmpBillingINS != 1)
                    {
                        convert2_xbrl.Show("Error while Inserting Finance Biling Info", this);
                    }

                    fianceShippingInformation();
                    tmpShippingINS = new clsResponse().financeShipingInfo(paymentID, sName, sAdd, sCity, sState, sZip, sTelephone);
                    if (tmpShippingINS != 1)
                    {
                        convert2_xbrl.Show("Error while Inserting Finance Shipping Info", this);
                    }

                    tmpActivate = new clsResponse().financeActivate(paymentID, bName, bEmail, createdDate, Quentity);
                    if (tmpActivate != 1)
                    {
                    ...
                    if (responseCode == 0)
                    {
                        ...
                        if (tmpCartDelete == 1)
                        {
                            ...
                            Response.Redirect("frmFinanceOrder.aspx?id=" + paymentID);
                        }
                        else
                        {
        ...
        public void fianceBillingInformation()
        {
            bName = Convert.ToString(SqlHelper.ExecuteScalar(ConfigurationManager.ConnectionStrings["converu6_xbrlEntities"].ToString(), CommandType.Text, "select billing_name from finance_checkout_data where user_id='" + tmpSessionUserID + "'"));
            bAdd = Convert.ToString(SqlHelper.ExecuteScalar(ConfigurationManager.ConnectionStrings["converu6_xbrlEntities"].ToString(), CommandType.Text, "select billing_address from finance_checkout_data where user_id='" + tmpSessionUserID + "'"));
            bCity = Convert.ToString(SqlHelper.ExecuteScalar(ConfigurationManager.ConnectionStrings["converu6_xbrlEntities"].ToString(), CommandType.Text, "select billing_city from finance_checkout_data where user_id='" + tmpSessionUserID + "'"));
            bState = Convert.ToString(SqlHelper.ExecuteScalar(ConfigurationManager.ConnectionStrings["converu6_xbrlEntities"].ToString(), CommandType.Text, "select billing_state from finance_checkout_data where user_id='" + tmpSessionUserID + "'"));
            bZip = Convert.ToInt32(SqlHelper.ExecuteScalar(ConfigurationManager.ConnectionStrings["converu6_xbrlEntities"].ToString(), CommandType.Text, "select billing_zip from finance_checkout_data where user_id='" + tmpSessionUserID + "'"));
            bEmail = Convert.ToString(SqlHelper.ExecuteScalar(ConfigurationManager.ConnectionStrings["converu6_xbrlEntities"].ToString(), CommandType.Text, "select billing_email from finance_checkout_data where user_id='" + tmpSessionUserID + "'"));
            bTelephone = Convert.ToString(SqlHelper.ExecuteScalar(ConfigurationManager.ConnectionStrings["converu6_xbrlEntities"].ToString(), CommandType.Text, "select billing_telephone from finance_checkout_data where user_id='" + tmpSessionUserID + "'"));
        }
        public void fianceShippingInformation()
        {
            sName = Convert.ToString(SqlHelper.ExecuteScalar(ConfigurationManager.ConnectionStrings["converu6_xbrlEntities"].ToString(), CommandType.Text, "select shipping_name from finance_checkout_data where user_id='" + tmpSessionUserID + "'"));
            sAdd = Convert.ToString(SqlHelper.ExecuteScalar(ConfigurationManager.ConnectionStrings["converu6_xbrlEntities"].ToString(), CommandType.Text, "select shipping_address from finance_checkout_data where user_id='" + tmpSessionUserID + "'"));
            sCity = Convert.ToString(SqlHelper.ExecuteScalar(ConfigurationManager.ConnectionStrings["converu6_xbrlEntities"].ToString(), CommandType.Text, "select shipping_city from finance_checkout_data where user_id='" + tmpSessionUserID + "'"));
            sState = Convert.ToString(SqlHelper.ExecuteScalar(ConfigurationManager.ConnectionStrings["converu6_xbrlEntities"].ToString(), CommandType.Text, "select shipping_state from finance_checkout_data where user_id='" + tmpSessionUserID + "'"));
            sZip = Convert.ToInt32(SqlHelper.ExecuteScalar(ConfigurationManager.ConnectionStrings["converu6_xbrlEntities"].ToString(), CommandType.Text, "select shipping_zip from finance_checkout_data where user_id='" + tmpSessionUserID + "'"));
            sTelephone = Convert.ToString(SqlHelper.ExecuteScalar(ConfigurationManager.ConnectionStrings["converu6_xbrlEntities"].ToString(), CommandType.Text, "select shipping_telephone from finance_checkout_data where user_id='" + tmpSessionUserID + "'"));
        }
        #endregion
    }
}